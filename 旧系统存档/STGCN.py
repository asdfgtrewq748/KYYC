...
def add_engineered_features(X, feature_names=None):
    ...
    if is_spatial:
        for feat_idx in range(F):
            ...
            # ⭐ 新增：变异系数（CV）- 使用更鲁棒的分母保护
            feat_cv = np.where(np.abs(feat_mean) > 1.0, 
                              feat_std / (feat_mean + 1e-8), 
                              0.0)  # 当均值接近0时，CV设为0
    ...
    if F > 1 and is_spatial:
        ...
        for feat_idx in range(1, min(F, 5)):  # 只取前5个特征避免过多
            ...
            # 只在分母绝对值大于0.1时计算比值，否则设为0
            ratio = np.where(np.abs(other_data) > 0.1, 
                           pressure_data / (other_data + 1e-8), 
                           0.0)
    ...
    # ⭐ 关键修复：强化数值稳定性处理
    # 步骤1: 先裁剪极端值，防止后续归一化时产生NaN
    X_enhanced = np.clip(X_enhanced, -1e6, 1e6)
    ...
    if inf_mask.any():
        print(f"⚠️ 警告：特征工程产生了 {inf_mask.sum()} 个Inf值，已替换为0")
        X_enhanced = np.nan_to_num(X_enhanced, nan=0.0, posinf=0.0, neginf=0.0)
    
    # 步骤3: 最终安全检查
    final_check_mask = ~np.isfinite(X_enhanced)
    if final_check_mask.any():
        print(f"⚠️ 最终检查：仍有 {final_check_mask.sum()} 个非有限值，已全部替换为0")
        X_enhanced[final_check_mask] = 0.0
    
    # 步骤4: 验证数值范围合理
    max_val = np.abs(X_enhanced).max()
    if max_val > 1e6:
        print(f"⚠️ 警告：特征值过大（max={max_val:.2e}），再次裁剪")
        X_enhanced = np.clip(X_enhanced, -1e6, 1e6)
    
    print(f"✓ 特征工程完成：特征范围 [{X_enhanced.min():.2f}, {X_enhanced.max():.2f}]")
...
use_geological = st.sidebar.checkbox(
    "✅ 使用地质特征", 
    value=True, 
    help="地质特征已在数据预处理阶段通过KDTree精确匹配"
)

geo_file = None
if use_geological:
    # 检查是否存在默认地质特征文件（仅用于验证）
    default_geo_path = os.path.join(os.path.dirname(__file__), 'geology_features_extracted.csv')
    use_default_geo = True  # 地质特征已在数据集中
    ...
    st.sidebar.success("✅ 地质特征已包含在数据集中")
    st.sidebar.info(
        """
        **包含的地质特征:**
        - 总厚度 (m)
    ...
    if not os.path.exists(default_geo_path):
        st.sidebar.warning("⚠️ 未找到 geology_features_extracted.csv")
        st.sidebar.info("请运行 preprocess/prepare_training_data.py 重新生成数据集")
else:
    st.sidebar.error("""
    ❌ **未启用地质特征**
...
if data_source == "使用预处理数据集" and npz_file:
    ...
    try:
        ...
        # 默认禁用特征工程
        use_feature_engineering = False  # 强制禁用，防止NaN
        ...
        **{model_type}**
        ...
        if st.button("🚀 开始训练", type="primary"):
            try:
                ...
                # ⭐ 地质特征已在数据预处理阶段完成
                # 数据集 sequence_dataset.npz 已包含正确匹配的地质特征
                # 不需要在此处再次融合
                
                # 显示地质特征信息
                if use_geological:
                    geo_feature_count = sum(1 for name in feature_names if name.startswith('geo_'))
                    if geo_feature_count > 0:
                        st.success(f"""
                        ✅ 地质特征已就绪！
                        - 地质特征数: {geo_feature_count} 个
                        - 来源: 预处理阶段通过 KDTree 精确匹配
                        - 每个支架已匹配到最近钻孔的地质特征
                        - 特征已包含在训练数据中
                        """)
                    else:
                        st.warning("""
                        ⚠️ 未检测到地质特征
                        - 请重新运行 preprocess/prepare_training_data.py
                        - 确保 geology_features_extracted.csv 存在
                        """)
